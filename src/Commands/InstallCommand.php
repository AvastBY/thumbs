			...
			if ($data_type && Schema::hasTable('data_rows')) {
				if(!DB::table('data_rows')->where('data_type_id', $data_type->id)->first()){
					$data = [
						...
						['data_type_id' => $data_type->id, 'field' => 'created_at', 'type' => 'timestamp', 'display_name' => 'Created At', 'required' => 0, 'browse' => 0, 'read' => 0, 'edit' => 0, 'add' => 0, 'delete' => 0, 'details' => '{}', 'order' => 10],
						['data_type_id' => $data_type->id, 'field' => 'updated_at', 'type' => 'timestamp', 'display_name' => 'Updated At', 'required' => 0, 'browse' => 0, 'read' => 0, 'edit' => 0, 'add' => 0, 'delete' => 0, 'details' => '{}', 'order' => 11],
					];

					DB::table('data_rows')->insert($data);
				}else{
					$data = [];
					if(!DB::table('data_rows')->where('data_type_id', $data_type->id)->where('field', 'blur')->first()) $data[] = ['data_type_id' => $data_type->id, 'field' => 'blur', 'type' => 'number', 'display_name' => 'Blur', 'required' => 0, 'browse' => 1, 'read' => 1, 'edit' => 1, 'add' => 1, 'delete' => 1, 'details' => '{}', 'order' => 8];
					if($data) DB::table('data_rows')->insert($data);
				}
			}
		}
